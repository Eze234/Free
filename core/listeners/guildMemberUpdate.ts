...
async function boost(
	...
	const channel = newMember.client.channels.cache.get(
		free.config.server.channel.boost
	) as TextChannel | undefined;
	const guild: Guild = newMember.guild;
	const boostRole: string | undefined =
		guild.roles.premiumSubscriberRole?.id ?? free.config.server.role.booster;
		...
		if (boostRole && newMember.roles.cache.has(boostRole)) {
			if (channel?.isSendable()) {
				...
				let message =
					...
					'%member% sybau 💔 https://cdn.ezezzz.xyz/v1/cdn/get/sybau';

				message = message
					.replace('%member%', `<@${newMember.id}>`)
					.replace('%count%', `${guild.premiumSubscriptionCount ?? 0}`);
